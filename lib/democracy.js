...
class Democracy extends EventEmitter {
  ...
  send(event, extra, id) {
    ...
    for (let x = 0; x < chunks.length; x += 1) {
      for (let i = 0; i < this.options.peers.length; i += 1) {
        const _sourceTemplate = `${this.options.peers[i][0]}:${this.options.peers[i][1]}`;
        if (!id || this._nodes[id].source === _sourceTemplate) {
          this.socket.send(
            chunks[x],
            0,
            chunks[x].length,
            this.options.peers[i][1],
            this.options.peers[i][0],
          );
        }
      }
    }
   ...
   */
  processEvent(msg) {
    const data = JSON.parse(decoder.write(msg));
